...
def train(input_tensor1, input_tensor2, input_tensor3, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    encoder_outputs1 = torch.zeros(max_length, encoder.hidden_size, device=device)
    encoder_outputs2 = torch.zeros(max_length, encoder.hidden_size, device=device)
    encoder_outputs3 = torch.zeros(max_length, encoder.hidden_size, device=device)
    encoder_outputs123 = torch.zeros(max_length, encoder.hidden_size, device=device)
    ...
    #ruvish = nn.GRU(encoder_outputs3[0].size(), encoder_outputs3[0].size())
    gruvish = nn.GRU(256,256)
    ...
    print("\n")
    print("printing aftre encoder outputs\n")
    print(encoder_outputs)
    print(encoder_outputs.size())'''
    ...
    gruvamc = nn.GRU(256,256)
    e = []
    e.append(encoder_hidden1)
    e.append(encoder_hidden2)
    e.append(encoder_hidden3)


    for t in range(3):
    	out123, encoder_hidden123 = gruvamc(e[t], encoder_hidden123)
...
def trainIters(encoder, decoder, n_iters, print_every=1000, plot_every=100, learning_rate=0.01):
    ...
    for iter in range(1, n_iters + 1):
        ...
        loss = train(input_tensor1, input_tensor2, input_tensor3, target_tensor, encoder,
...
def evaluate(encoder, decoder, sentence, max_length=MAX_LENGTH):
    with torch.no_grad():
    	...
    	sent3 = ' '.join(sentence.split(' ')[2*l+1:3*l+1])
...
trainIters(encoder1, attn_decoder1, 5, print_every=100)
...
evaluateAndShowAttention("inter more ross and")
